...
public class PaymentAppService : IPaymentAppService
{
    ...
    public async Task<ServiceResult<PaymentDto>> CreatePaymentAsync(int orderId, CreatePaymentDto dto)
    {
        ...
        // Le montant est impose par le serveur pour eviter toute modification client.
        var paymentAmount = order.TotalPrice;
        var created = await _payments.AddPaymentAsync(
            Payment.CreateCompleted(orderId, paymentAmount, (PaymentMethod)dto.Method, DateTime.UtcNow));
